...
### v0.6 — 群聊考古 + 可视化
- **群聊热搜榜**：微博热搜格式，话题排行+热度指数+上榜天数
- **关系可视化**：群友互动节点图（谁互动最多、关系类型）
- **群友考古**：第一条发言、最搞笑发言、"X年前的今天"

### v0.7 — 周报/月报 + 在线模型（脱敏架构）
- **周报/月报**：以日报摘要为输入（非原始聊天），在线模型做深度总结
- **在线模型配置**：支持 OpenAI/Claude/DeepSeek API
- **脱敏设计**：本地 14B 消化原始聊天 → 产出结构化的日报摘要 → 在线模型只看到摘要（话题/情绪/关键词），看不到原始对话内容和群友名字

### v0.8 — 年度盛典 + 智能问答
- **年度颁奖典礼**：10+ 奖项，AI 写颁奖词
- **智能问答**：基于日报摘要的群聊问答
