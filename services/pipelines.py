...
PROMPTS = {
    ...
    "quotes": {
        "system": "你是一个金句挖掘工具。格式：[N]|原话|你的吐槽，每行一条。群友用[N]格式引用。不要输出任何其他内容。",
        "user": "{chat}\n\n找出 3-5 条最搞笑/精彩的发言。输出格式（严格按此格式，发言人用[N]）：\n[N]|原话|你的吐槽\n示例：\n[1]|我昨天梦到自己在写代码|社畜的自我修养\n[13]|今天天气真好，适合摸鱼|说出了大家的心声\n\n你的输出：",
    },
    "mood": {
    ...
    "oneline": {
        "system": "你是一个群聊总结工具。先在心里回顾今天的聊天主线，然后严格输出两行：第一行总结，第二行高光时刻。群友用[N]格式引用。不要加序号或前缀。",
        "user": "{chat}\n\n第一行：用20字内有梗的话总结今天群聊（群友用[N]格式）\n第二行：今天最值得记录的瞬间（群友用[N]格式）\n示例：\n[1]和[13]红包大战的周五\n[1]连发10个红包手速惊人\n\n你的输出：",
    },
    "active_hours": {
...
def _parse_oneline(raw) -> tuple[str, str]:
    ...
    # 截断过长内容
    return one_line[:60], highlight[:120]


def _parse_active_hours(raw) -> dict:
    """解析活跃时段，适配扁平化输出（字符串或旧版嵌套对象）"""
    text = str(raw).strip() if raw else ""
    if not text:
        return {"peak_label": "全天在线", "peak_desc": ""}
    # 新版：直接返回字符串，匹配枚举值
    PEAKS = {
        "上午摸鱼": "上午摸鱼", "午间活跃": "午间活跃", "下午茶话会": "下午茶话会",
        "晚间热闹": "晚间热闹", "夜猫子专场": "夜猫子专场", "凌晨修仙": "凌晨修仙",
        "全天在线": "全天在线",
    }
    peak_label = ""
    ...
    if not peak_label:
        # 模糊匹配
        for kw, label in [("上午", "上午摸鱼"), ("中午", "午间活跃"), ("下午", "下午茶话会"),
                          ("晚上", "晚间热闹"), ("夜间", "夜猫子专场"), ("凌晨", "凌晨修仙"),
                          ("通宵", "凌晨修仙"), ("全天", "全天在线")]:
            if kw in text:
                peak_label = label
                break
    if not peak_label:
        peak_label = "全天在线"
    return {"peak_label": peak_label, "peak_desc": text if text != peak_label else "", "quiet_note": ""}
...
DAILY_ONLINE_SYSTEM = """你是一个群聊观察员，每天为群聊写日报。
风格要求：
- 像给朋友讲八卦一样生动有趣，不要像写工作周报
- 吐槽要犀利但有分寸，不攻击个人
- 发现群友的可爱之处，记录有趣的瞬间
...
注意：speaker、one_line、highlight 等所有涉及群友的地方，都用 [数字] 格式，如 [1]吐槽[13]的穿搭。"""
...
性格标签参考（可自由发挥）：
- 较真、随和、社恐、自来熟、刀子嘴、老好人、玻璃心、佛系
- 急性子、细节控、强迫症、摆烂王、卷王、乐子人、吃货、愤青、文艺

说话风格参考：
- 豪爽、温柔、毒舌、理性、活泼、老成、阴阳怪气、一本正经
- 骚话连篇、惜字如金、直球、拐弯抹角

群内角色参考：
- 气氛组、和事佬、话题制造机、话题终结者、吃瓜群众
- 潜水大佬、毒舌评论员、科普达人、摸鱼冠军、卷王

emoji标签参考：
- 乐天派、整活王、暖心、暴脾气、老学究、派对咖、emo怪、沙雕
- 吃瓜群众、摸鱼达人、摆烂王、卷王、老司机、玻璃心、凡尔赛
- 社死选手、夜猫子、游戏宅、美食家、潜水冠军、捧场王、毒舌、话痨、文艺青年
