...
def _extract_period_raw_data(
    ...
    stats = {
        ...
        "top_speakers": [{"sender_id": m["sender_id"], "count": m["msg_count"]}
                        for m in ranked_members[:5]],
        "night_owls": [{"sender_id": m["sender_id"], "peak": f"{m['peak_hour']}:00"}
                      for m in night_owls[:3]],
        "lurkers": [{"sender_id": m["sender_id"], "days": m["days_active"], "msgs": m["msg_count"]}
                   for m in lurkers[:3]],
        "emoji_kings": [{"sender_id": m["sender_id"], "emojis": m["top_emojis"]}
...
async def _do_ai_generate(system_prompt: str, user_prompt: str,
    ...
    # v1.19.x: 在线模型使用 PipelineContext 统一日志
