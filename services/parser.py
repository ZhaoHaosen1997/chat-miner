...
def _get_stable_id(wxid: str, senders: list[dict]) -> int:
    ...
    v1.18.5: 委托给 desensitize.build_wxid_to_stable_id（权威实现），
    本地缓存 keyed by sorted wxid tuple 避免每条消息重建映射。
    """
    if not senders:
        return 0
    ...
    if key not in _stable_id_cache:
        ...
        _stable_id_cache[key] = build_wxid_to_stable_id(senders)
...
_KEEP_MESSAGE_KEYS = {"wxid", "senderID", "createTime", "formattedTime", "type", "content", "platformMessageId"}
...
def _normalize_qq_time(time_str: str) -> str:
    """将 QQ 导出的各种 time 格式统一为 YYYY-MM-DD HH:MM:SS

    QQ 群聊 chunked JSONL 的 time 为 ISO 8601 格式 (2025-11-09T13:38:53.000Z)，
    QQ 私聊 single JSON 的 time 为标准格式 (2024-09-04 18:02:20)，
    需要统一为下游代码期望的 YYYY-MM-DD HH:MM:SS。
    """
    if not time_str:
        return ""
    if 'T' in time_str:
        # ISO 8601: "2025-11-09T13:38:53.000Z" → "2025-11-09 13:38:53"
        dt_part = time_str.replace('T', ' ').replace('Z', '')
        if '.' in dt_part:
            dt_part = dt_part.split('.')[0]
        return dt_part
    return time_str  # 已经是标准格式


def _safe_extract_text(m: dict) -> str:
    """安全提取 QQ 消息的文本内容

    QQ 'content' 字段可能是: None (JSON null)、str (直接字符串)、
    dict (含 text 键的标准格式)。只有 dict 有 .get('text')。
    """
...
# 图片/视频等无意义占位文本（仅哈希/文件名，无实际可读内容）
_QQ_PLACEHOLDER_PATTERN = re.compile(
    r'^\[(图片|视频|文件|语音|表情|合并转发|转发消息)[\s:：].*\]$'
)


def _is_placeholder_content(text: str) -> bool:
    """判断 QQ 消息文本是否仅为媒体占位符（无实际聊天内容）"""
    return bool(_QQ_PLACEHOLDER_PATTERN.match(text.strip()))


def _clean_group_card(group_card: str) -> str:
    """清理 QQ 群名片中的 [有人@我] 等提示前缀"""
    if not group_card:
        return ""
    return re.sub(r'^\[有人@我\]', '', group_card).strip()
...
def _normalize_qq(raw: dict) -> tuple[dict, list[dict], list[dict]]:
    ...
    for m in qq_messages:
        ...
        sid_to_info[next_sid] = {
            "senderID": next_sid,
            "wxid": uid,
            "displayName": _clean_group_card(sender.get("groupCard", "")) or sender.get("nickname") or sender.get("remark") or sender.get("name") or "",
    ...
    for m in qq_messages:
        ...
        content_text = _safe_extract_text(m)
        # 过滤占位文本和无文本内容的消息（纯图片/文件等），不参与 AI 分析
        if not content_text or _is_placeholder_content(content_text):
            continue
        ...
        messages.append({
            ...
            "formattedTime": _normalize_qq_time(m.get("time", "")),
            "type": internal_type,
            ...
            for line in chunk_data.splitlines():
                ...
                if uid not in uid_to_sid:
                    ...
                    sid_to_info[next_sid] = {
                        ...
                        "wxid": uid,
                        "displayName": (
                            _clean_group_card(sender.get("groupCard", ""))
                ...
                content_text = _safe_extract_text(m)
                # 过滤占位文本和无文本内容的消息（纯图片/文件/视频占位符）跳过
                if not content_text or _is_placeholder_content(content_text):
                    continue
...
class ParsedChat:
        ...
        if self._pickle_path.exists() and (not json_exists or self._pickle_path.stat().st_mtime >= json_mtime):
            try:
                ...
                if pickle_mtime >= json_mtime:
                    ...
                    # 1. 检测旧版 ISO 时间格式 → 自动修复
                    if self.messages and 'T' in self.messages[0].get('formattedTime', ''):
                        logger.info("检测到旧版 ISO 时间格式，自动修复 %d 条消息...", len(self.messages))
                        for m in self.messages:
                            m['formattedTime'] = _normalize_qq_time(m.get('formattedTime', ''))
                        needs_repair = True

                    # 2. 缺失 platform → 根据数据特征自动推断
                    if not self.platform:
                        self.platform = self._infer_platform()
                        if self.platform:
                            logger.info("自动推断 platform=%s (群=%s)", self.platform, self.group_name)
                            needs_repair = True
                    ...
                    if needs_repair:
                        try:
                            ...
                            logger.debug("存量 pickle 已修复并回写: %s", self._pickle_path)
                        except Exception as e:
                            logger.warning("回写修复后 pickle 失败: %s", e)
    ...
    def _infer_platform(self) -> str:
        """根据数据特征推断平台类型 (v1.8.5 存量兼容)

        推断优先级：
        1. sender wxid 以 'u_' 开头 → QQ
        2. 群 wxid 为纯数字 → QQ
        3. 群 wxid 含 '@chatroom' → 微信
        4. 含 wxid 以 'wxid_' 开头 → 微信
        5. 默认空字符串
        """
        # 检查 sender wxid 模式
        if self.senders:
            sample_wxids = [s.get("wxid", "") for s in self.senders[:5] if s.get("wxid")]
            if sample_wxids:
                u_count = sum(1 for w in sample_wxids if w.startswith("u_"))
                if u_count >= len(sample_wxids) * 0.6:
                    return "qq"
        # 检查群 wxid 模式
        gw = self.group_wxid
        if gw:
            ...
            if "@chatroom" in gw:
                return "wechat"
            if gw.startswith("wxid_"):
                return "wechat"
            if gw.startswith("wxid_"):
                return "wechat"
        # 检查消息 wxid 模式
        if self.messages:
            sample_msg_wxids = [m.get("wxid", "") for m in self.messages[:10] if m.get("wxid")]
            if sample_msg_wxids:
                u_count = sum(1 for w in sample_msg_wxids if w.startswith("u_"))
                if u_count >= len(sample_msg_wxids) * 0.6:
                    return "qq"
        return ""
