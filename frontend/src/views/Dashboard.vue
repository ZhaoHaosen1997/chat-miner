        ...
        <div v-if="dayPopup" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" @click.self="dayPopup = null">
          <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 max-h-[80vh] overflow-y-auto p-6">
            ...
            <div class="space-y-3">
              ...
              <div v-else-if="dayPopup.hasData" class="bg-amber-50 rounded-xl p-4 text-center"><p class="text-sm text-amber-700">该日期尚未分析</p><p v-if="dayPopupError" class="text-xs text-red-500 mb-2">{{ dayPopupError }}</p><button @click="handleGenerateReport()" :disabled="aiBusy || !!dayPopupLoading" class="mt-2 text-xs bg-amber-500 text-white px-3 py-1.5 rounded-lg hover:bg-amber-600 transition-colors disabled:opacity-40">立即分析</button></div>
            </div>
          </div>
        </div>
      </Teleport>
